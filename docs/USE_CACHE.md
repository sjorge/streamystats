...
When `cacheComponents: true` is enabled, you **cannot** use these route segment configs:
...
**Solution**: Remove these exports from route files. API routes default to Node.js runtime and work correctly without explicit config.
...
## Dynamic Data Restriction

**You cannot use `cookies()` or `headers()` inside `"use cache"` functions.** These read from the incoming request, which would invalidate the cache on every request.
...
async function getData() {
  "use cache";
  const isLoggedIn = (await cookies()).has('token'); // ERROR
  // ...
}
```
...
Resolve dynamic data *before* calling the cached function and pass it as an argument:

```ts
async function getDataCached(userId: string) {
  "use cache";
  cacheLife("hours");
...
export async function getData() {
  const session = await getSession(); // calls cookies() here
  if (!session) return [];
  return getDataCached(session.id); // userId becomes part of cache key
}
```

This pattern separates the dynamic request context from the cached computation.
...
## Combining with "use server"

Files can have `"use server"` at the top level while individual functions use `"use cache"`. This is useful when:

- The file contains server actions (mutations)
- Some functions need caching, others don't
- You need to prevent client bundling of server-only code
